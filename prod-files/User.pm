...
sub auth_api_safe {
    ...
    if ( $otp->get_enabled($user) ) {
        ...
        unless ( $otp->verify_token( $otp->get_secret($user), $args{otp_token} ) ) {
            ...
            unless ( $backup_valid ) {
                report->status( 401 );
                report->add_error('INVALID_OTP_TOKEN');
                return undef;
            }
        }
...
sub auth {
    ...
    my $user_row = $self->_find_user_by_login_or_email( $args{login} );
    ...
    return undef unless $user_row->{password} eq $password;
...
sub _find_user_by_login_or_email {
    my $self = shift;
    my $login_or_email = shift;

    return undef unless $login_or_email;

    my ( $user_row ) = $self->_list(
        where => { login => $login_or_email },
        limit => 1,
    );
    return $user_row if $user_row;

    return undef unless is_email( $login_or_email );

    ( $user_row ) = $self->_list(
        where => {
            sprintf('%s->>"$.%s"', 'settings', 'email') => $login_or_email,
        },
        limit => 1,
    );
...
sub api_password_auth_status {
    ...
    return {
        ...
        email => $self->_get_effective_email,
    };
}
...
sub api_set_email {
    ...
    unless ( is_email($email) ) {
        $report->add_error('INVALID_EMAIL');
        return undef;
    }

    # Check settings.email uniqueness
    my ( $existing ) = $self->_list(
        where => {
            sprintf('%s->>"$.%s"', 'settings', 'email') => $email,
        },
        limit => 1,
    );

    if ( $existing && $existing->{user_id} != $self->id ) {
        $report->add_error('EMAIL_ALREADY_EXISTS');
        return undef;
    }

    # Check login column uniqueness (another user may have this email as login)
    my ( $login_match ) = $self->_list(
        where => { login => $email },
        limit => 1,
    );

    if ( $login_match && $login_match->{user_id} != $self->id ) {
        $report->add_error('EMAIL_ALREADY_EXISTS');
        return undef;
    }
...
sub _get_effective_email {
    my $self = shift;

    my $settings_email = $self->get_settings->{email};
    return $settings_email if $settings_email;

    my $login = $self->get_login;
    return $login if is_email($login);
