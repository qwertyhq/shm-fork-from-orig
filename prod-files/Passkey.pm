...
sub api_delete {
    ...
    # Guard: don't remove last passkey if it's the only auth method
    my @credentials = @{$self->get_credentials($user)};
    if ( scalar @credentials <= 1 && $user->is_password_auth_disabled ) {
        my $has_telegram = $user->get_settings->{telegram} && $user->get_settings->{telegram}{user_id};
        unless ( $has_telegram ) {
            $report->add_error('CANNOT_DELETE_ONLY_AUTH_METHOD');
            return undef;
        }
    }
