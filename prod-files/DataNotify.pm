        ...
        *{"Core::Sql::Data::$method"} = sub {
            ...
            eval {
                ...
                $self->add_post_commit_callback(sub {
                    ...
                    # Сбрасываем dedup-ключ ПОСЛЕ публикации. На HTTP-пути config->local
                    # пересоздаётся на каждый запрос (shm-server.pl), но в долгоживущих
                    # воркерах (spool и т.п.) — нет. Без этого delete ключ ws:$table:$user_id
                    # остаётся навсегда, и все ПОСЛЕДУЮЩИЕ изменения той же пары (продления,
                    # повторный блок/разблок, сбросы трафика) перестают публиковаться до
                    # перезапуска воркера. callbacks выполняются и очищаются в commit
                    # (Core::Sql::Data::commit), так что delete срабатывает ровно один раз.
                    delete $pending->{$key};
                });
            };
