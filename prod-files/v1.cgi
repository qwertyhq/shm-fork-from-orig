...
'/user/email' => {
                        ...
                        'application/json' => {
                            schema => {
                                ...
                                properties => {
                                    email => {
                                        type => 'string',
                                        description => 'Email пользователя (пустая строка если не привязан)',
                                    },
                                },
                            },
                        },
    ...
    POST => {
        ...
        required => ['email'],
        swagger => {
            summary => 'Установить email для авторизации',
            description => 'Привязать email к аккаунту. Email должен быть уникальным. После привязки можно входить по email+пароль.',
            responses => {
                '200' => {
                    content => {
                        'application/json' => {
...
'/telegram/link' => {
    ...
    POST => {
        controller => 'Transport::Telegram',
        method => 'api_link_telegram',
        swagger => {
            summary => 'Привязать Telegram аккаунт к текущему пользователю',
            description => 'Принимает данные от Telegram Login Widget (id, first_name, last_name, username, photo_url, auth_date, hash). Проверяет HMAC-подпись, уникальность telegram_id и сохраняет привязку.',
            responses => {
                '200' => {
                    content => {
                        'application/json' => {
...
'/telegram/unlink' => {
    ...
    POST => {
        controller => 'Transport::Telegram',
        method => 'api_unlink_telegram',
        swagger => {
            summary => 'Отвязать Telegram аккаунт от текущего пользователя',
            description => 'Удаляет привязку Telegram из настроек пользователя. После этого вход через Telegram станет невозможен.',
            responses => {
                '200' => {
                    content => {
                        'application/json' => {
