...
sub api_link_telegram {
    ...
    my $hash = delete $in{hash};
    return report->error('Missing required Telegram fields') unless $in{id} && $hash && $in{auth_date};
    ...
    unless ($hex eq $hash) {
        report->error('Telegram signature verification failed');
        $self->set_user_fail_attempt('link_telegram', 3600, $self->telegram_ips);
        return undef;
    }

    return report->error('Telegram auth data too old') if time - $in{auth_date} > 86400;

    my $telegram_id = $in{id};

    # Check if this telegram_id is already linked to another user
    my ($existing) = $self->user->_list(
        where => {
            -OR => [
                sprintf('%s->>"$.%s"', 'settings', 'telegram.user_id') => $telegram_id,
                login => get_shm_login($telegram_id),
                sprintf('%s->>"$.%s"', 'settings', 'telegram.chat_id') => $telegram_id,
            ],
        },
        limit => 1,
    );

    my $current_user = get_service('user');

    if ($existing && $existing->{user_id} != $current_user->id) {
        return report->error('TELEGRAM_ALREADY_LINKED');
    }

    # Save telegram data to current user's settings
...
sub api_unlink_telegram {
    my $self = shift;

    my $user = get_service('user');
    my $settings = $user->get_settings;

    return report->error('TELEGRAM_NOT_LINKED') unless $settings->{telegram} && $settings->{telegram}{user_id};

    # Ensure user has alternative auth method before unlinking
    my $has_password = !$user->is_password_auth_disabled && $user->_get_effective_email;
    my $has_passkey = get_service('Passkey')->get_enabled($user);

    unless ( $has_password || $has_passkey ) {
        return report->error('CANNOT_UNLINK_ONLY_AUTH_METHOD');
    }
