...
{{ END }}
{{# BLOCK для Web-интерфейса (устраняет дублирование) #}}
{{ BLOCK web_interface_buttons }}
[
    {
...
[
    {
        "text": "⚙️ Сменить интерфейс",
        "callback_data": "/show_interface_menu"
    }
]
{{ END }}
{{# BLOCK для answerCallbackQuery (убирает "часики" на кнопках) #}}
{{ BLOCK answer_callback }}
{
    "answerCallbackQuery": {
...
<% CASE 'USER_NOT_FOUND' %>
{{ TEXT = BLOCK }}
Добро пожаловать в бот сети HQ VPN! 
...
{
    "sendMessage": {
        "text": "⚠️ <b>Вы уже использовали триальный период.</b>\n\nДля продолжения использования VPN, пожалуйста, оформите подписку.\n\nОткройте Web-приложение:",
        "parse_mode": "HTML",
        "reply_markup": {
            "inline_keyboard": [
                [
...
{{ IF cmd == '/menu' }}
{
    "editMessageText": {
        "message_id": {{ message.message_id }},
        "text": "🌐 Вы используете Web-интерфейс. Откройте приложение:",
        "parse_mode": "HTML",
        "reply_markup": {
            "inline_keyboard": [
                [
...
{{ IF args.0 == 'web' }}
{
    "editMessageText": {
        "message_id": {{ message.message_id }},
        "text": "🌐 Вы переключились на Web-интерфейс. Откройте приложение:",
        "parse_mode": "HTML",
        "reply_markup": {
            "inline_keyboard": [
                [
...
{{ ELSE }}
{
    "editMessageText": {
        "message_id": {{ message.message_id }},
        "text": "🤖 Вы переключились на Бот-интерфейс. Сейчас откроется главное меню:",
        "parse_mode": "HTML",
        "reply_markup": {
            "inline_keyboard": [
                [
...
{{ buttons = BLOCK }}
                ...
                [
                    {
                        "text": "🤖 Переключиться на Бот-интерфейс",
                ...
                [
                    {
                        "text": "🌐 Переключиться на Web-интерфейс",
        ...
        {{ IF service.status == 'ACTIVE' }}
            {{ IF service.category.grep('^vpn-m-%').first }}
                ...
                {{ ELSE }}
                    [
                        {
                            "text": "ОШИБКА: Напишите администратору",
                            "callback_data": "/menu"
                        }
                    ],
                    [
...
{{ ELSIF message.text }}
{
    "sendMessage": {
        "chat_id": "{{ tpl.settings.admin.chat_id }}",
"parse_mode": "HTML",
        "text": "<b>Пишет {{ user.full_name }}-#{{ user.settings.telegram.chat_id }}#:</b>\n\n{{ message.text }}"
    }
}
{{ ELSE }}
{
