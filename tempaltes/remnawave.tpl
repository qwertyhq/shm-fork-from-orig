...
├─ us.service.settings.data_limit - лимит трафика в GB (по умолчанию: 500, 0 = безлимит)
├─ us.service.settings.unlimited_traffic - true для безлимитного трафика
...
БЕЗЛИМИТНЫЙ ТРАФИК:
├─ UNLIMITED_SERVICE_IDS - массив ID услуг с безлимитом (захардкожен в шаблоне)
│  └─ Текущие: [28, 29] - семейные тарифы
└─ Приоритет: ID в списке → unlimited_traffic=true → data_limit=0 → лимит из настроек → 500GB
...
# Список ID услуг с безлимитным трафиком (семейные тарифы)
{{ UNLIMITED_SERVICE_IDS = [28, 29] }}

# Безлимитный трафик: service_id в списке ИЛИ unlimited_traffic=true ИЛИ data_limit=0 → trafficLimitBytes=0
{{ current_service_id = us.service_id + 0 }}
{{ is_in_unlimited_list = 0 }}
{{ FOREACH sid IN UNLIMITED_SERVICE_IDS }}
  {{ IF (sid + 0) == current_service_id }}
    {{ is_in_unlimited_list = 1 }}
  {{ END }}
{{ END }}
...
{{ IF is_in_unlimited_list }}
  {{ data_limit = 0 }}
  {{ is_unlimited = 1 }}
{{ ELSIF us.service.settings.unlimited_traffic }}
  {{ data_limit = 0 }}
  {{ is_unlimited = 1 }}
{{ ELSIF us.service.settings.data_limit == 0 || us.service.settings.data_limit == '0' }}
  {{ data_limit = 0 }}
  {{ is_unlimited = 1 }}
{{ ELSIF us.service.settings.data_limit }}
  {{ data_limit = us.service.settings.data_limit * 1073741824 }}
  {{ is_unlimited = 0 }}
{{ ELSE }}
  {{ data_limit = 500 * 1073741824 }}
  {{ is_unlimited = 0 }}
{{ END }}
{{ reset_strategy = us.service.settings.data_limit_reset_strategy || 'MONTH' }}
{{ NAME_PREFIX = config.remnawave.name_prefix || "HQVPN_" }}

# Приоритет HWID лимита: услуга → семейный тариф (10) → глобальная конфигурация → по умолчанию (5)
{{ IF us.service.settings.hwid_limit != '' && us.service.settings.hwid_limit != null }}
  {{ HWID_LIMIT = us.service.settings.hwid_limit + 0 }}
{{ ELSIF is_in_unlimited_list }}
  {{ HWID_LIMIT = 10 }}
...
{{ IF is_unlimited }}
log_debug "DATA_LIMIT: UNLIMITED (trafficLimitBytes=0)" # Для отладки
{{ ELSE }}
