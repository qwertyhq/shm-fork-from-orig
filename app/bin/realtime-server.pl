...
# browser clients connected via WebSocket or SSE, routed by user_id.
#
...
my %clients;       # fileno => { sock, user_id, buf, handshake_done, type }
...
sub accept_client {
    ...
    $select->add($csock);
    log_msg("new client fd=$fn from ", $csock->peerhost // 'unknown');
}
...
sub read_client {
    ...
    # SSE clients are read-only — incoming data is ignored,
    # disconnect is detected by sysread returning 0 (handled above)
}
...
sub do_handshake {
    ...
    # Detect protocol: WebSocket upgrade or SSE
